...
def analyze_farm(reading: FarmReading, user: dict = Depends(verify_firebase_token)):
    ...
    if temp >= 40.0:
        insights.append(f"Barn temperature is critically high at {temp}°C — severe heat stress zone (≥40°C).")
        recommendations.append("Activate cooling system immediately and increase ventilation.")
    elif temp > 37.0:
        insights.append(f"Barn temperature ({temp}°C) is above the pig comfort zone (34–37°C).")
        recommendations.append("Monitor pigs closely. Consider pre-cooling or misting systems.")
    elif temp < 32.0:
        insights.append(f"Barn temperature is critically low at {temp}°C — chilling risk.")
        recommendations.append("Provide supplemental heating and check for drafts.")
    elif temp < 34.0:
        insights.append(f"Barn temperature ({temp}°C) is below the pig comfort zone (34–37°C).")
        recommendations.append("Check insulation and heating. Monitor pigs for huddling or lethargy.")
    else:
        insights.append(f"Barn temperature ({temp}°C) is within the optimal comfort zone (34–37°C).")
